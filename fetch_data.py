...
def fetch_workshare_difficulty(config: dict[str, Any], data_dir: Path) -> str:
    ...
    QiCompute's inference layer uses GPUs only. Two workshare buckets are tracked:
      - KawPoW (GPU) workshares below block difficulty threshold
      - TWP (Tensor Work Proof) inference workshares — native Quai algorithm

    Each workshare has a ``difficulty`` field (hex) that represents the work
    done. We aggregate these into daily series:
    ...
    Algorithm identification heuristic:
    ...
    Until the RPC exposes an explicit algo tag, these buckets are best-effort
    and the limitation is documented in the output.
    ...
    if not daily_kawpow and not daily_twp:
        raise ValueError("workshare_difficulty: rpc scan produced no samples")

    source = (
        f"{url} (rpc_scan workshares, step={step}; "
        "algo split: kawpow_ws=has-mixHash, twp_ws=has-twpReceipt/tensorReceipt; "
    ...
    n_twp = len(daily_twp)
    return (
        f"workshare_difficulty: {n_kw} days kawpow_ws, "
